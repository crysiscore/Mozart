...
mysql -uroot  -ppassword  <  fill_hdd/fill_hhd_chamanculo.sql
...
SELECT location_id,NAME,address2,county_district,state_province,location_id FROM openmrs.location l WHERE county_district='Cidade de Maputo'; 
 SELECT location_id,NAME,address2,county_district,state_province,location_id FROM openmrs.location  NAME like '%manculo%';
...
UPDATE global_property SET property_value = 210 WHERE property='mozart.location';



use openmrs;
-- set @ydata := '2021-12-20';
-- set @sismaLocationID := 1110214; -- cs_chamanculo
set @openmrsID :=210;
